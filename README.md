...
![Image](https://github.com/user-attachments/assets/e65ef6b5-ac39-400e-bdb9-968ca0deba04)
  ...
  • 결제수단(현재 카드결제 기능만 구현)
    - **포트원 api 연동**
    - 외부 결제 시스템(pg사) : 이지페이(kicc) 사용
    
  • 결제완료 후 검증
    - 결제 후 클라이언트에 AJAX로 결제 완료 전달 클래스 호출, 포트원에서의 결제 정보 서버에 전송
    - 서버 측 검증 및 DB 저장
    - 클라이언트가 보낸 금액과 포트원 서버에서 조회한 실제 결제 금액을 비교
    - 금액 일치 여부, 상태 확인 후 DB에 결제 내역 저장
    - 검증 성공 응답을 받은 클라이언트가 장바구니 상태 갱신, 주문 목록 생성
    -"결제 완료" 알림 및 주문 목록 페이지로 이동
