...
.info-section h2, .editor-section h2 {
	font-size: 18px;
	color: #333;
	margin-bottom: 15px;
...
.size-quantity-item input[type="text"], .size-quantity-item input[type="number"]
	{
	padding: 8px;
	border: 1px solid #ced4da;
	border-radius: 4px;
...
.product-option-item input[type="text"], .product-option-item input[type="number"]
	{
	flex-grow: 1;
}
		...
		<div class="info-section">
			...
			<div class="product-options-section">
				...
				<div id="productOptionsContainer">
					...
					<div class="product-option-item"
						data-item-index="${optionItemIndex}">
						<div class="option-item-header">
							<h3>옵션 #${optionItemIndex + 1}</h3>
							<button type="button" class="btn-remove-option-item">항목
								삭제</button>
						</div>
						<div class="form-group">
							<label for="option_color_${optionItemIndex}">색상 정보:</label> <input
								type="text" name="options_color[]"
								id="option_color_${optionItemIndex}" placeholder="예: 빨강">
						</div>
						<div class="form-group">
							<label for="option_size_${optionItemIndex}">사이즈 정보:</label> <input
								type="text" name="options_size[]"
								id="option_size_${optionItemIndex}" placeholder="예: M">
						</div>
						<div class="form-group">
							<label for="option_quantity_${optionItemIndex}">상품 수량:</label> <input
								type="number" name="options_quantity[]"
								id="option_quantity_${optionItemIndex}" placeholder="예: 10"
								value="0" min="0">
						</div>
					</div>
			...
			<div class="button-group">
				<button type="button" class="btn btn-secondary"
					onclick="submitContents(this);">임시저장</button>
				<button type="button" class="btn btn-primary"
					onclick="submitContents(this);">상품 등록하기</button>
			</div>
	</form>
	...
	<script>
     ...
     // --- 새로운 상품 옵션 (색상, 사이즈, 수량 세트) 동적 관리 ---
            ...
            const optionItemHtml = `
                <div class="product-option-item" data-item-index="${optionItemIndex}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="options_color[]" id="option_color_${optionItemIndex}" placeholder="예: 빨강">
                    </div>
                    <div class="form-group">
                        <label for="option_size_${optionItemIndex}">사이즈 정보:</label>
                        <input type="text" name="options_size[]" id="option_size_${optionItemIndex}" placeholder="예: M">
                    </div>
                    <div class="form-group">
                        <label for="option_quantity_${optionItemIndex}">상품 수량:</label>
                        <input type="number" name="options_quantity[]" id="option_quantity_${optionItemIndex}" placeholder="예: 10" value="0" min="0">
                    </div>
                </div>
            `;
      ...
      function submitContents(elClickedObj) {
       ...
       // 2. 상품 정보 입력(2) - 상품 옵션 유효성 검사 (수정 필요)
          // 이제 input name이 'options_color[]' 등이 되었으므로
          // 아래처럼 전체를 순회하면서 검사하는 방식은 동일하게 적용 가능합니다.
          if ($("input[name='options_color[]']").length === 0) {
            alert(
