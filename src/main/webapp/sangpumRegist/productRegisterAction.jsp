...
try {
    ...
    if (productIdStr != null && !productIdStr.trim().isEmpty()) {
        try {
            ...
            System.err.println("Invalid productId format: " + productIdStr); // 유효하지 않은 productId는 신규 등록으로 간주
            productId = 0;
        }
    }
    ...
    if (colors == null || colors.length == 0 || sizes == null || sizes.length == 0 || quantities == null || quantities.length == 0 || colors.length != sizes.length || sizes.length != quantities.length) {
    ...
    for (int i = 0; i < colors.length; i++) {
        ...
        optionList.add(optionDto); // 'optionList'에 추가
    }

    // 7. DAO를 통해 데이터베이스에 저장
    ProductDao productDao = new ProductDao();
    productDao.saveProduct(productDto, optionList);

    // 수정 모드인지 신규 등록 모드인지에 따라 메시지 설정
    if (productId > 0) {
...
// Java에서 메시지를 JavaScript에 안전하게 전달하기 위해 이스케이프 처리
String escapedMessageForJs = "";
if (message != null) {
    // 백슬래시를 먼저 이스케이프하고, 그 다음 작은따옴표를 이스케이프
    escapedMessageForJs = message.replace("\\", "\\\\").replace("'", "\\'");
}
%>

<script type="text/javascript">
    var alertMessage = "<%= escapedMessageForJs %>";
    if (alertMessage) {
        alert(alertMessage);
    }
    ...
    var contextPath = "<%= request.getContextPath() %>"; // 예: /SemiProject

    if (redirectPath.startsWith("/")) {
        // 컨텍스트 내의 절대 경로
        targetUrl = contextPath + redirectPath;
    } else {
        // 상대 경로 (./로 시작하거나 파일 이름만 있는 경우)
        var currentPath = window.location.pathname; // 예: /SemiProject/sangpumRegist/productRegisterAction.jsp
        var lastSlashIndex = currentPath.lastIndexOf("/");
        var basePath = currentPath.substring(0, lastSlashIndex); // 예: /SemiProject/sangpumRegist

        if (redirectPath.startsWith("./")) {
            targetUrl = basePath + "/" + redirectPath.substring(2); // "./" 제거
        } else {
            // 예: productListAdmin.jsp
